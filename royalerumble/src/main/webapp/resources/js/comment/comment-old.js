...
$.fn.commentsPanel = function (opt, list) {
    var $commentPanel = {
        header : $(commentTempl.header(opt.total)),
    ...
    $commentPanel.header.find('.comment-write').append(commentTempl.write(opt.userName, 'comment'));
...
$.fn.commentList = function (list, mode) {
    ...
    list.forEach(comment=> {
        template += commentTempl.comment(comment);
    });
    switch (mode){
        case 'make' :
...
function replies(opt, $replies, ref, mode) {
    ...
    opt.api.list(params, response => {
        ...
        $replies.reply.children('.reply-write').append(commentTempl.write(opt.userName, 'reply'));
    });
}
...
$.fn.replyList = function (opt, list, mode) {
    ...
    var template = '';
    list.forEach(reply => {
        template += commentTempl.reply(reply);
    });
    switch (mode){
        case 'make' :
